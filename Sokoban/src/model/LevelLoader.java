...
public class LevelLoader {
    ...
    public GameObjects getLevel(int level){
        int currentLevel = level % 60 == 0 ? level : level % 60;
        ...
        for (int i = 0; i < levelInfo.size(); i++) {
            ...
            for (int j = 0; j < wight.length(); j++) {
                int x = (j == 0) ? Model.FIELD_CELL_SIZE / 2 :
                        (Model.FIELD_CELL_SIZE / 2) + j * Model.FIELD_CELL_SIZE;
                int y = (i == 0) ? Model.FIELD_CELL_SIZE / 2 :
                        (Model.FIELD_CELL_SIZE / 2) + i * Model.FIELD_CELL_SIZE;
                ...
                switch (symbol) {
                    ...
                    case '.' :
                        ...
                        break;
                    case '&' :
                        boxes.add(new Box(x, y, 12, 12));
                        homes.add(new Home(x, y));
                        break;
                    case '@' :
                        player = new Player(x, y, 11, 11);
                        break;
                }
            }
        }
